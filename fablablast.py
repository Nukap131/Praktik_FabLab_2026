...
print("FABLAB PERSON TÆLLER V2.5 (HORIZONTAL - KUN IND - GROUP IMPROVED)")

# ---------------- RELÆ ----------------

RELAY_PIN = 17
RELAY_PULSE_SEC = 0.2
RELAY_COOLDOWN_SEC = 0.8
last_relay_time = 0.0
...
# Hvis en bbox er meget bred, kan det være 2 personer tæt sammen.
group_width_px = 180

# Hvor mange loop et track skal ignoreres efter count
track_cooldown_frames = 12

# ---------------- HELPERS ----------------

def insert_in_event(timestamp, tid):
    global total_crossings, current_inside
...
try:
    while True:
        ...
        for i in range(int(num)):
            ...
            width_px = int((xmax - xmin) * frame_width)
            ...
            # Simpelt gruppe-hack:
            # Hvis boksen er meget bred, tilføj en ekstra "virtuel" detection.
            # Det hjælper lidt når 2 personer overlapper og bliver til 1 bred boks.
            if width_px > group_width_px:
                detections.append({
                    "cy": cy + 8,   # lille offset så den ikke er 100% identisk
                    "width_px": width_px,
                    "score": score
                })
        ...
        for det in detections:
            ...
            if best_tid is None:
                ...
                # Mindre smoothing = hurtigere respons
        ...
        for tid, track in tracks.items():
            ...
            # KUN IND: TOP -> BOTTOM
            if track["start_side"] == "TOP" and track["seen_top"] and track["seen_bottom"]:
                insert_in_event(timestamp, tid)
                track["counted"] = True
                track["cooldown"] = track_cooldown_frames
